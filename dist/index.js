...
var availableSlots = mysqlTable("availableSlots", {
  id: int("id").autoincrement().primaryKey(),
  ...
  // HH:mm
  maxAppointments: int("maxAppointments").default(1).notNull(),
  currentAppointments: int("currentAppointments").default(0).notNull(),
...
async function getAvailableSlots(date) {
  ...
  if (!db) return [];
  try {
    return await db.select().from(availableSlots).where(and(
...
async function updateAppointmentStatus(id, status, googleCalendarEventId) {
  const db = await getDb();
  if (!db) return;
  try {
...
async function createGoogleCalendarEvent(eventTitle, eventDescription, startDateTime, endDateTime, attendeeEmail) {
  try {
...
async function sendConfirmationEmailToClient(clientEmail, clientName, appointmentDate, appointmentTime, formLink) {
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      <h2 style="color: #333;">Agendamento Confirmado! \u{1F389}</h2>
      <p>Ol\xE1 <strong>${clientName}</strong>,</p>
      <p>Seu agendamento foi confirmado com sucesso!</p>
      
      <div style="background-color: #f5f5f5; padding: 20px; border-radius: 8px; margin: 20px 0;">
        <h3 style="color: #333; margin-top: 0;">Detalhes do Agendamento</h3>
        <p><strong>Data:</strong> ${formattedDate}</p>
        <p><strong>Hor\xE1rio:</strong> ${appointmentTime}</p>
      </div>

      ${formLink ? `
        <div style="margin: 20px 0;">
          <p>Pr\xF3ximo passo: Complete o formul\xE1rio da segunda etapa clicando no bot\xE3o abaixo.</p>
          <a href="${formLink}" style="display: inline-block; background-color: #007bff; color: white; padding: 12px 24px; text-decoration: none; border-radius: 4px; font-weight: bold;">
            Ir para Formul\xE1rio
          </a>
        </div>
      ` : ""}

      <p style="color: #666; font-size: 14px; margin-top: 30px;">
        Se voc\xEA tiver d\xFAvidas, entre em contato conosco.
      </p>
      
      <hr style="border: none; border-top: 1px solid #ddd; margin: 20px 0;">
      <p style="color: #999; font-size: 12px;">
        Este \xE9 um e-mail autom\xE1tico. N\xE3o responda diretamente.
      </p>
    </div>
  `;
  return sendEmail({
...
async function sendNotificationEmailToAdmin(adminEmail, clientName, clientEmail, clientPhone, appointmentDate, appointmentTime) {
  const formattedDate = appointmentDate.toLocaleDateString("pt-BR", {
    weekday: "long",
    year: "numeric",
    month: "long",
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      <h2 style="color: #333;">Novo Agendamento Recebido \u{1F4C5}</h2>
      
      <div style="background-color: #f5f5f5; padding: 20px; border-radius: 8px; margin: 20px 0;">
        <h3 style="color: #333; margin-top: 0;">Informa\xE7\xF5es do Cliente</h3>
      ...
      <p style="color: #666; font-size: 14px; margin-top: 30px;">
        Voc\xEA pode acompanhar todos os agendamentos no painel administrativo.
      </p>
...
var appRouter = router({
  ...
  appointments: router({
    ...
    getAvailableSlots: publicProcedure.input(z2.object({ date: z2.string() })).query(async ({ input }) => {
    ...
    create: publicProcedure.input(
      z2.object({
        ...
        endTime: z2.string(),
        // HH:mm
        googleFormsLink: z2.string().optional()
      })
    ).mutation(async ({ input, ctx }) => {
      try {
        const startDateTime = /* @__PURE__ */ new Date(`${input.appointmentDate}T${input.startTime}:00`);
        const endDateTime = /* @__PURE__ */ new Date(`${input.appointmentDate}T${input.endTime}:00`);
        const appointment = await createAppointment({
          userId: ctx.user?.id || 1,
          // Fallback para usuário padrão se não autenticado
          name: input.name,
          email: input.email,
          phone: input.phone,
...
Per\xEDodo: ${input.startTime} \xE0s ${input.endTime}`,
            startDateTime,
            endDateTime,
            input.email
          );
        ...
        try {
          await sendConfirmationEmailToClient(
            ...
            input.name,
            startDateTime,
            `${input.startTime} \xE0s ${input.endTime}`,
